...
<div class="wrap">
  ...
  <p class="foot">build 19 Sep 2026 v6. Source: Bosch eBike Flow ride summaries pulled through the rider activity API. Heart rate is not recorded because the bike has no sensor. Climbing is metres of elevation gain. Times are Melbourne local time.</p>
</div>
...
<main>
  <div id="map"><div class="stamp">build 19 Sep 2026 v6</div></div>
  <aside class="side">
...
function hideTip(){ tip.hidden=true; }
function closeAllTips(){ hideTip(); map.eachLayer(l=>{ if(l.closeTooltip) l.closeTooltip(); }); }
map.on('click movestart zoomstart',closeAllTips);
...
function gpxText(s,opt){ const pts=resolveW(s[opt].w); const esc=t=>String(t).replace(/&/g,'&amp;').replace(/</g,'&lt;'); const name=esc(s.name+' ('+opt+')');
  let x='<?xml version="1.0" encoding="UTF-8"?>\n<gpx version="1.1" creator="Nic\'s Ride Map" xmlns="http://www.topografix.com/GPX/1/1">\n<metadata><name>'+name+'</name></metadata>\n';
  pts.forEach(p=>{ x+='<wpt lat="'+p[0].toFixed(5)+'" lon="'+p[1].toFixed(5)+'"><name>'+esc(p[2]||'')+'</name></wpt>\n'; });
  x+='<rte><name>'+name+'</name>\n'; pts.forEach(p=>{ x+='<rtept lat="'+p[0].toFixed(5)+'" lon="'+p[1].toFixed(5)+'"><name>'+esc(p[2]||'')+'</name></rtept>\n'; }); x+='</rte>\n';
  x+='<trk><name>'+name+'</name><trkseg>\n'; pts.forEach(p=>{ x+='<trkpt lat="'+p[0].toFixed(5)+'" lon="'+p[1].toFixed(5)+'"></trkpt>\n'; }); x+='</trkseg></trk>\n</gpx>\n'; return x; }
function gpxFile(s,opt){ return new File([gpxText(s,opt)],(s.id+'_'+opt+'.gpx'),{type:'application/gpx+xml'}); }
function gpx(s,opt){ const a=document.createElement('a'); a.href=URL.createObjectURL(new Blob([gpxText(s,opt)],{type:'application/gpx+xml'})); a.download=(s.id+'_'+opt+'.gpx'); document.body.appendChild(a); a.click(); a.remove(); }
async function sendToFlow(s,opt,btn){ const f=gpxFile(s,opt);
  if(navigator.canShare && navigator.canShare({files:[f]})){ try{ await navigator.share({files:[f],title:s.name}); btn.textContent='Sent. In the share sheet choose the Flow app.'; return; }catch(e){ if(e&&e.name==='AbortError') return; } }
  gpx(s,opt); btn.textContent='GPX saved. Open it with the Flow app to import.'; }
...
function exploreDetail(s,opt){
  ...
  const fl=h('button','btn',acts,'Send to the Flow app'); fl.addEventListener('click',()=>sendToFlow(s,opt,fl));
  const d=h('button','btn',acts,'Download GPX'); d.addEventListener('click',()=>gpx(s,opt));
  h('p','why',explore,'Send to the Flow app opens your phone\'s share sheet with the route file. Pick the Bosch eBike Flow app and it imports the route into Navigation, where you get the map and turn by turn directions on the bike. Flow works out the actual streets and paths between the waypoints.');
...
if(new URLSearchParams(location.search).get('embed')==='1') document.body.classList.add('embed');
function setMode(mode){ const b=document.querySelector('.chip[data-mode="'+(mode==='explore'?'explore':'rides')+'"]'); if(b&&b.getAttribute('aria-pressed')!=='true') b.click(); closeAllTips(); setTimeout(()=>map.invalidateSize(),50); }
